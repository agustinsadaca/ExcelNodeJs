...
const calculateModifiedDate = (date) => {
  ...
  const currentDateFormated =
    currentMonthWithZeros + "/" + currentDay + "/" + currentYear;
...
const writeExcel = (jsonExcel, filename) => {
  try {
    ...
    for (let index = 0; index < jsonExcel.length; index++) {
      ...
      worksheet
        .cell(index + 2, 1)
        .string("Cliente generico de Prisma")
        .style(style);
      worksheet
        .cell(index + 2, 2)
        .string("1")
      ...
      worksheet
        .cell(index + 2, 17)
        .string(`0004-${jsonExcel[index]["NUM.CUPON"]}`)
        .style(style);
      worksheet
        .cell(index + 2, 19)
        .string("FB")
      ...
      worksheet
        .cell(index + 2, 40)
        .number(parseFloat(jsonExcel[index]["MONTO_BRUTO"]) / 1.21)
        .style(style);
    ...
    direccionExcel = `download/Comprobantes${filename.replace(
      ".csv",
      ""
    )}.xlsx`;
...
const writeExcelBancoNacion = (jsonExcel, filename) => {
  try {
    var workbook = new excel.Workbook();
    var worksheet = workbook.addWorksheet("COMPROBANTES");
    var style = workbook.createStyle({
    ...
    for (let index = 0; index < jsonExcel.length; index++) {
      ...
      worksheet
        .cell(index + 2, 1)
        .string("Cliente generico de Banco Nacion")
        .style(style);
      worksheet
        .cell(index + 2, 2)
        .string("1")
      ...
      worksheet
        ...
        .number(
          parseFloat(
            jsonExcel[index].Monto.replace(/[^0-9,]/g, "").replace(",", ".")
          ) / 1.21
        )
        .style(style);
      const todayDate = Date.now();
...
module.exports = { writeExcel, writeExcelBancoNacion };
